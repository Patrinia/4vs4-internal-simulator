...
public interface IUnitBrain
{
    ...
    /// [업데이트] 최신 전황 파악을 위해 FormationManager를 주입받습니다.
    /// </summary>
    /// <param name="usableSkills">현재 쿨타임이 돌고 있지 않은 사용 가능 스킬들</param>
    /// <param name="allUnits">전장에 존재하는 모든 유닛 (피아식별 및 타겟팅 용도)</param>
    /// <param name="formationManager">1차원 진형 관리자 (위치 기반 타겟팅 용도)</param>
    /// <returns>선택된 스킬과 타겟 유닛 리스트가 담긴 DTO (사용 가능 스킬이 없으면 null 반환)</returns>
    ActionDecision SelectNextSkill(List<SkillData> usableSkills, List<UnitControl> allUnits, FormationManager formationManager);
}